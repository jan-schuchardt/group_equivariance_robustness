...
class DimeNetPP(torch.nn.Module):
    ...
    def forward(self, z, pos, batch=None):
        ...
        if self.predict_forces:
            F = -torch.autograd.grad(P.sum(), pos, create_graph=True)[0] # retain_graph = True
...
class OutputBlockAlternative(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, out_emb_size, out_channels, num_layers,
                 act=swish):
        super().__init__()
        self.act = act
...
class DimeNetPP_alternative(torch.nn.Module):
    r"""
    ...
    def __init__(self, hidden_channels: int, out_channels: int,
                 ...
                 mean=None, stddev=None):
        super().__init__()
        print("\n\n\n " + "=="*10 + "USING DIMENET ALTERNATIVE" + "==" * 10 + "\n\n\n")
        self.cutoff = cutoff
        self.max_num_neighbors = max_num_neighbors
        self.num_blocks = num_blocks
        ...
        self.output_blocks = torch.nn.ModuleList([
            OutputBlockAlternative(num_radial, hidden_channels, out_emb_size, out_channels,
                        num_output_layers, act) for _ in range(num_blocks + 1)
        ])
