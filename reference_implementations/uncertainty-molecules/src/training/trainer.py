...
from src.models.interfaces import LightningInterface
import pytorch_lightning as pl
import torch
from src.eval_functions import test_ood
...
class ModelWrapper(pl.LightningModule):
    ...
    def evaluate_ood(self):
        ...
        for idx, batch in enumerate(self.test_dataloader()):
            loss = self.test_step(batch.to(self.device), idx)
        metrics = test_ood(self.model, self.val_dataloader(), 
                           self.test_dataloader(), 'ood_set')
        self.log_metrics("ood", metrics)
