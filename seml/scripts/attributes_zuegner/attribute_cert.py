...
from equivariance_robustness.attributes_zuegner.certification import \
    certify_ged
...
def config():
    ...
    seed = 0
    save_dir = '/nfs/staff-hdd/schuchaj/equivariance_certification_results/attributes_zuegner'

train = ex.capture(train, prefix='training_params')
certify_ged = ex.capture(certify_ged, prefix='certification_params')
...
def main(_config, seed, hidden_sizes, certification_params, save_dir):
    ...
    for Q in tqdm(range(1, certification_params['Q_max']+1)):
        certified_at_Q = certify_ged(gcn_model=model, attrs=X.astype("float32"), q=q, 
